...
interface CoffeeComponent {
    fun provideElectricHeater(): ElectricHeater
    fun provideGasHeater(): GasHeater
}

class DI {
    fun main() {
        val coffeeComponent = DaggerCoffeeComponent.create()
        val electricCoffeeMachine = coffeeComponent.provideElectricCoffeeMachine()
        val gasCoffeeMachine = coffeeComponent.provideGasCoffeeMachine()
...
class ElectricCoffeeMachine @Inject constructor(private val heater: ElectricHeater) : CoffeeMachine {
...
class GasCoffeeMachine @Inject constructor(private val heater: GasHeater) : CoffeeMachine {
    override fun makeCoffee(type: CoffeeType, ingredients: List<Ingredient>) {
        val ingredient = ingredients.joinToString(", ")
        Log.e(TAG, "makeCoffee: Making a ${type.name} coffee with $ingredient")
